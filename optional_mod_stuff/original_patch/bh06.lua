...
-- Cutscenes --
CUTSCENE_IN =						"br06-1.bik"
CUTSCENE_OUT =						"br06-2.bik"
...
function bh06_start(bh06_checkpoint, is_restart)
	...
	if (is_restart) then
		...
		cutscene_play(	"br06-01", start_groups, {NAV_LOCAL_START, NAV_REMOTE_START}, false)
		for i,group in pairs(start_groups) do
			group_show(group)
		end		
	end
...
function bh06_get_to_arena()
	...
	mission_end_success("bh06", "br06-02", {NAV_LOCAL_END, NAV_REMOTE_END})
...
function bh06_failure_truck_destroyed()
	...
	delay(3)
	mission_end_failure("bh06", TEXT_PARCEL_TRUCK_DESTROYED)
end
...
function bh06_failure_pierce_died()
	mission_end_failure("bh06", TEXT_PIERCE_DIED)
end

function bh06_failure_pierce_dismissed()
	mission_end_failure("bh06", TEXT_PIERCE_DISMISSED)
end

function bh06_failure_didnt_get_to_arena_in_time()
	mission_end_failure("bh06", TEXT_DIDNT_GET_TO_ARENA)
end
