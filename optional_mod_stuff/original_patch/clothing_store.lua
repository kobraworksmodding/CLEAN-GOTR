...
function clothing_store_build_item_list_menu(menu_data)
	...
	if Clothing_store_is_wardrobe == true and pcu_is_underwear_category(Clothing_store_current_category) == false then
		local bra_required = pcu_bra_required()
		if bra_required == false or pcu_is_bra_category(Clothing_store_current_category) == false then		
			menu_data.num_items = 1
...
function clothing_store_discard_item()
	...
	if pcu_is_current_item_worn(Clothing_store_item_info.slot_index, Clothing_store_item_info.wardrobe_instance) then
		...
		if pcu_is_underwear_category(Clothing_store_current_category) then
			dialog_box_message("STORE_UNDERWEAR_WARNING","STORE_UNDERWEAR_SUB_WARNING", true, true)
		else 
			if pcu_is_bra_category(Clothing_store_current_category) and pcu_bra_required() then
				dialog_box_message("STORE_BRA_WARNING", "STORE_BRA_SUB_WARNING")
			else
