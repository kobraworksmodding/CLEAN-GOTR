...
function hud_healthbars_init()
	...
	debug_print("vint", "Initializing hud healthbars\n")
	...
	--mp player cash, unmoveable
	Hud_healthbars_status.handles.mp_player_cash_h = vint_object_find("mp_player_cash")
	
	-- Start this out empty
	vint_set_property( Hud_healthbars_status.handles.mp_player_cash_h, "text_tag", "" )
	...
	--mp_player_cash fade out
	Hud_healthbars_status.handles.mp_player_cash_fade_out = vint_object_find("mp_player_cash_fade_out")
	...
	--mp_player_cash fade out
	vint_set_property(Hud_healthbars_status.handles.mp_player_cash_fade_out, "is_paused", true)
	...
	-- MP player cash subscription
	vint_dataitem_add_subscription("sr2_sa_mp_player_cash", "update", "mp_player_cash_update")
end
...
function hud_healthbars_update(di_h, event)
	...
	local no_object_key, object_handle, screen_x, screen_y, z_depth,
		...
		is_visible, color, is_big, health_full_alpha,
		show_bar, show_name, show_arrow, show_cash, show_race_pos = vint_dataitem_get(di_h)
	...
	if event == "update" then
		...
		if Hud_healthbars_status.hostiles[key] == nil then
			...
			if is_big then
				...
				vint_set_property(hostile.fill_h, "image", "ui_hud_meter_fill")
				vint_set_property(hostile.border_h, "image", "ui_hud_meter_border")
				vint_set_property(hostile.grit_h, "image", "ui_hud_meter_grit")	
			end
...
--			debug_print( "vint_healthbars", "Creating healthbar for " .. decoded_name .. " key: " .. key .. "\n" )
		...
		if is_big then
			...
			vint_set_property(hostile.border_h, "source_se", (w * health_pct) + 4, h + 4)
		else
			--regular bar
...
--	debug_print( "vint_healthbars", "Current healthbar entries:\n" )
	...
	for key, entry in Hud_healthbars_status.hostiles do
--		debug_print( "vint_healthbars", "\tkey: " .. key .. " name: " .. entry.decoded_name .. "\n" )
	end
end

function mp_player_cash_update( di_h, event )
	local cash = vint_dataitem_get( di_h )
	
	-- Tint the text blue
	tint_element( Hud_healthbars_status.handles.mp_player_cash_h, COLOR_BLUE )
	
	if cash > 0 then
		vint_set_property( Hud_healthbars_status.handles.mp_player_cash_h, "text_tag", "+$" .. format_cash( cash ) )
		
		vint_set_property( Hud_healthbars_status.handles.mp_player_cash_h, "visible", true )
		vint_set_property( Hud_healthbars_status.handles.mp_player_cash_h, "alpha", 1 )
		vint_set_property( Hud_healthbars_status.handles.mp_player_cash_fade_out, "is_paused", true )
	else
		if Hud_healthbars_status.mp_player_cash > 0 then
			lua_play_anim( Hud_healthbars_status.handles.mp_player_cash_fade_out )
		end
	end
	
	Hud_healthbars_status.mp_player_cash = cash
end
