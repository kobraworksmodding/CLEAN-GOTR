...
function rn01_start(rn01_checkpoint, is_restart)
	...
	if(rn01_checkpoint == CHECKPOINT_START) then
		...
		if (not is_restart) then
			cutscene_play("ro01-01", group_list, {NAVPOINT_LOCAL_PLAYER_START, NAVPOINT_REMOTE_PLAYER_START}, false)
...
function rn01_complete()
	...
	mission_end_success("rn01", "ro01-02")
end
...
function rn01_failure_gat_died()
	...
	mission_end_failure("rn01", HELPTEXT_FAILURE_GAT_DIED)
end

function rn01_failure_gat_abandoned()
	-- End the mission, Gat abandoned
	mission_end_failure("rn01", HELPTEXT_FAILURE_GAT_ABANDONED)
end

function rn01_failure_pierce_died()
	-- End the mission, Pierce died
	mission_end_failure("rn01", HELPTEXT_FAILURE_PIERCE_DIED)
end

function rn01_failure_pierce_abandoned()
	-- End the mission, Pierce abandoned
	mission_end_failure("rn01", HELPTEXT_FAILURE_PIERCE_ABANDONED)
end

function rn01_failure_truck_destroyed()
	-- End the mission, Gat abandoned
	delay(2.0)
	mission_end_failure("rn01", HELPTEXT_FAILURE_TRUCK_DESTROYED)
end
