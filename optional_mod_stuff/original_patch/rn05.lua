...
function rn05_start(rn05_checkpoint, is_restart)
	...
	if (rn05_checkpoint == CHECKPOINT_START) then
		...
		if (is_restart) then
			...
			cutscene_play("ro05-01", start_groups, {NAVPOINT_LOCAL_PLAYER_START,NAVPOINT_REMOTE_PLAYER_START}, false)
			for i,group in pairs(start_groups) do
				group_show(group)
			end
...
function rn05_the_chase(start_from_chase)
	...
	cutscene_play("IG_rn05_scene1", "", "", false)

	-- Destroy the old group.
	group_destroy(GROUP_RONIN_CHASE)

	-- Create a new group.
	group_create(GROUP_RONIN_CHASE, true)
...
function rn05_complete()
	...
	mission_end_success("rn05", "ro05-02", {NAVPOINT_END_SUCCESS, NAVPOINT_END_SUCCESS})
end
