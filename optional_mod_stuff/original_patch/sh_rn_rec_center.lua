...
--	trigger_enable("sh_rn_rec_center_$televator", true) --!!!!TEMP!!! this is normally called after the johns are killed	
	...
	if (not is_restart) then
		cutscene_play("sh_rn_rec_center_ct1")	
	end
	--
	--cutscene_play("strongholds_rec_scene2")
	--delay(10)
...
--mission_debug("weapon=  "..weapon)
	...
	mission_end_failure("sh_rn_rec_center","MSN_SH_RN_REC_CENTER_FAIL_BETRAY")
	end
...
function sh_rn_rec_center_casino_entrance()
	...
	on_random_ods_killed("sh_rn_rec_center_casino_killcount", "sh_rn_rec_center")	
	while total_destroyed < destroyed_target do	
		thread_yield()	
	end		
		on_random_ods_killed("", "sh_rn_rec_center")
...
function sh_rn_rec_center_casino_2()
	...
	trigger_enable("sh_rn_rec_center_$tcasino_2", false)
	on_random_ods_killed("sh_rn_rec_center_casino_killcountTwo", "sh_rn_rec_center")
	objective_text(0, "sh_rn_rec_center_casinoTwo", total_destroyed, destroyed_target)
end
...
function sh_rn_rec_center_killcount(char)
	...
	delay(3)
	mission_end_success("sh_rn_rec_center")	
	end
end
...
function sh_rn_rec_center_failure()
	mission_end_failure("sh_rn_rec_center","sh_rn_rec_center_fail")
end
