...
-- Added by IdolNinja. These variables are used in the script for the cutscenes for stability instead of calling them directly
	...
	CUTSCENE_IN = 		"sh_bh_chinatown_ct1"
	CUTSCENE_MID =		"sh_bh_chinatown_ct2"
	MISSION_NAME =		"sh_bh_chinatown"
	MISSION_FAIL_WARNING =	"msn_sh_bh_chinatown_warning_fail"
	MISSION_FAIL =		"sh_bh_chinatown_failure"
...
function sh_bh_chinatown_start(checkpoint, is_restart)
	...
	on_random_ods_killed("sh_bh_chinatown_mover_death", MISSION_NAME)
...
function sh_bh_chinatown_timer(char)
	...
	if coop_is_active() then
			...
			while(in_area==false) do
				...
				if (time > 15) then					
					--mission_help_table_nag("sh_bh_chinatown_warning_one")
					mission_end_failure(MISSION_NAME, MISSION_FAIL_WARNING)
				end				
...
function sh_bh_chinatown_in_area( triggerer_name, trigger_name )	
	...
	on_random_ods_killed("sh_bh_chinatown_mover_death", MISSION_NAME)		--Set HUD range	
...
mission_end_failure(MISSION_NAME, "sh_bh_chinatown_warning_fail")
