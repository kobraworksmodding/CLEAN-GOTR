...
-- Added by IdolNinja. These variables are used in the script for the cutscenes for stability instead of calling them directly
	...
	CUTSCENE_IN = 		"sh_bh_airport"
	MISSION_NAME =		"sh_bh_airport"
	MISSION_FAIL_PLANES =	"sh_bh_airport_fail_planes"
	MISSION_FAIL =		"sh_bh_airport_warning_fail"
		...
		plane_tracker = plane_index
			...
			delay(60) -- This delay is how much time after a plane starts taking off ansd its not destroyed a failure will register
				...
				if(vehicle_is_destroyed(planes[plane_tracker])) then
					...
					mission_end_failure(MISSION_NAME,MISSION_FAIL_PLANES)
				end
...
function sh_bh_airport_failure_warning()
	mission_end_failure(MISSION_NAME,MISSION_FAIL) -- make failure`
end
