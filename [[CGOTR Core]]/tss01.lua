...
CUTSCENE_IN = 		"tssp01-01"
CUTSCENE_OUT = 		"tssp01-02"
MISSION_NAME =		"tss01"
...
function tss01_start(checkpoint, is_restart)
	...
	if (checkpoint == MISSION_START_CHECKPOINT) then
		...
		if (is_restart) then
			...
			cutscene_play(CUTSCENE_IN, start_groups, {LOCAL_PLAYER_START,REMOTE_PLAYER_START}, false)
			for i,group in pairs(start_groups) do
				group_show(group)
			end
		end
...
function tss01_failure_carlos()
	...
	mission_end_failure(MISSION_NAME, "tss01_carlos_died")
end

function tss01_abandon_carlos()
	-- End the mission since Carlos was abandoned
	mission_end_failure(MISSION_NAME, "tss01_carlos_abandoned")
end

function tss01_failure_yacht()
	-- End the mission since the yacht was destroyed
	mission_end_failure(MISSION_NAME, "tss01_yacht_destroyed")
end
...
function tss01_keep_ar200_equipped()
	...
	local function update_player(player)
		if (not inv_item_is_equipped("AR200", player)) and character_is_in_vehicle(player, HIGH_SEAS_VEHICLE) then
			inv_item_equip("AR200", player)
		end
...
--	end
	...
	delay(3.0)
	-- Success!
	mission_end_success(MISSION_NAME,CUTSCENE_OUT, {LOCAL_PLAYER_FINISH, REMOTE_PLAYER_FINISH})
end
