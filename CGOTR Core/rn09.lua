...
function rn09_start(rn09_checkpoint, is_restart)
	...
	if (rn09_checkpoint == CHECKPOINT_START) then
		if (is_restart) then
			...
			cutscene_play(CUTSCENE_IN, starting_groups, {NAVPOINT_LOCAL_PLAYER_START, NAVPOINT_REMOTE_PLAYER_START}, false)
			for i,group in pairs(starting_groups) do
				group_show(group)
			end
		end
...
function rn09_shogo_escaped_failure()
	...
	mission_end_failure(MISSION_NAME, HELPTEXT_FAILURE_SHOGO_FLED)
end
...
function rn09_gat_dismiss_failure()
	-- End the mission, fence died
	mission_end_failure(MISSION_NAME, HELPTEXT_FAILURE_GAT_DISMISSED)
end
