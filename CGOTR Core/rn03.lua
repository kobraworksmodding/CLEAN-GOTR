...
function rn03_start(rn03_checkpoint, is_restart)
	...
	if(rn03_checkpoint == CHECKPOINT_START) then
		if (not is_restart) then
			cutscene_play(CUTSCENE_IN, "", {NAVPOINT_LOCAL_PLAYER_START, NAVPOINT_REMOTE_PLAYER_START}, false)
		else
...
function rn03_failure_bike_destroyed()
	mission_end_failure(MISSION_NAME, HELPTEXT_FAILURE_BIKE_DESTROYED)
end

-- End the mission, Ronin made it to the saint's district
function rn03_failure_ronin_in_row()
	mission_end_failure(MISSION_NAME, HELPTEXT_FAILURE_RANGE)
end
